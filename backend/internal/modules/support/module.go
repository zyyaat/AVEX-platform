// Package support is the composition root for the support module.
package support
...
import (
	...
	"avex-backend/internal/modules/support/repository/postgres"
	"avex-backend/internal/modules/support/service"
	httptransport "avex-backend/internal/modules/support/transport/http"
...
func New(cfg *config.Config, pool *pgxpool.Pool, logger *slog.Logger) *Module {
	...
	deps := port.Deps{
		...
		TicketNumberGen: &ticketNumberGen{},
...
func (m *Module) NewInbox() inbox.Inbox {
	return &inboxAdapter{
		inner: inbox.NewPostgresInbox(m.pool, inbox.Config{Table: "support.inbox"}),
	}
}
...
type ticketNumberGen struct{}

func (*ticketNumberGen) Generate() string {
	return "TKT-" + time.Now().UTC().Format("20060102") + "-" + newUUID()[:8]
}
