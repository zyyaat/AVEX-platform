// Package audit is the composition root for the audit module.
package audit
...
import (
	...
	"avex-backend/internal/modules/audit/events"
	"avex-backend/internal/modules/audit/port"
	"avex-backend/internal/modules/audit/repository/postgres"
	"avex-backend/internal/modules/audit/service"
	httptransport "avex-backend/internal/modules/audit/transport/http"
...
func New(cfg *config.Config, pool *pgxpool.Pool, logger *slog.Logger) *Module {
	...
	deps := port.Deps{
		Clock: &realClock{}, IDGenerator: &uuidIDGen{}, EventPublisher: eventPublisher,
		Logger: loggerAdapter{logger}, TxRunner: txRunner, Repos: repoSet,
	}
...
func (m *Module) NewInbox() inbox.Inbox {
	return &inboxAdapter{inner: inbox.NewPostgresInbox(m.pool, inbox.Config{Table: "audit.inbox"})}
}
...
type inboxAdapter struct{ inner *inbox.PostgresInbox }
func (a *inboxAdapter) IsProcessed(ctx context.Context, eventID, handlerName string) (bool, error) { return a.inner.IsProcessed(ctx, eventID, handlerName) }
func (a *inboxAdapter) MarkProcessed(ctx context.Context, eventID, handlerName, eventType string) error { return a.inner.MarkProcessed(ctx, eventID, handlerName, eventType) }
func (a *inboxAdapter) MarkProcessedTx(ctx context.Context, _ inbox.DBTX, eventID, handlerName, eventType string) error { return a.inner.MarkProcessed(ctx, eventID, handlerName, eventType) }
