...
func RegisterRoutes(mux *http.ServeMux, svc port.ServicePort, logger *slog.Logger, jwtIssuer idp.JWTIssuer) {
        ...
        // Admin: list ALL restaurants (including inactive) — same handler, different path
        mux.Handle("GET /api/v1/admin/restaurants", authMW(http.HandlerFunc(h.ListRestaurants)))
