...
func (s *Subscriber) handleAssignmentRequested(ctx context.Context, envelope bus.EventEnvelope) error {
	...
	// Use the delivery location as both pickup and delivery for now.
	// In production, we'd lookup the restaurant location from the catalog module.
	// For now, use delivery coords for both (the matching finds nearest driver to delivery).
	if err := s.svc.HandleOrderAssignmentRequested(
		ctx,
		payload.OrderID,
		payload.ZoneID,
		payload.DeliveryLat, payload.DeliveryLng, // pickup (will be replaced by restaurant location in production)
		payload.DeliveryLat, payload.DeliveryLng, // delivery
	); err != nil {
