// Tests for the admin API client (src/lib/api.ts)
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { setAuthToken, getAuthToken, adminAuthAPI, adminAPI } from './api'
...
describe('auth token storage', () => {
  it('persists the token under avex_admin_token', () => {
    setAuthToken('abc123')
    expect(localStorage.getItem('avex_admin_token')).toBe('abc123')
  ...
  it('removes the token when cleared', () => {
    setAuthToken('abc123')
    setAuthToken(null)
    expect(localStorage.getItem('avex_admin_token')).toBeNull()
...
describe('apiFetch behaviour', () => {
  it('uses the /api/v1 base and unwraps { data: ... }', async () => {
    fetchMock.mockResolvedValueOnce(
      mockResponse({ body: { data: { token: 't1', user: { is_admin: true } } } })
    )

    const result = await adminAuthAPI.login({ phone: '0100', password: 'pw' })
  ...
  it('extracts the error message from an { error } body', async () => {
    ...
    await expect(adminAuthAPI.me()).rejects.toThrow('boom')
  })

  it('dashboard falls back to safe defaults on network failure (no crash)', async () => {
