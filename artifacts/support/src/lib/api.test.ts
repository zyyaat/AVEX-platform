// Tests for the support API client (src/lib/api.ts)
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { setAuthToken, getAuthToken, agentAuthAPI, agentAPI } from './api'

function mockResponse(init: { status?: number; body?: unknown }) {
  const status = init.status ?? 200
  return {
    status,
    ok: status >= 200 && status < 300,
    text: async () => (init.body === undefined ? '' : JSON.stringify(init.body)),
    json: async () => init.body,
  } as unknown as Response
}

const fetchMock = vi.fn()

beforeEach(() => {
  vi.stubGlobal('fetch', fetchMock)
  fetchMock.mockReset()
  localStorage.clear()
  setAuthToken(null)
})

describe('auth token storage', () => {
  it('persists the token under avex_agent_token', () => {
    setAuthToken('abc123')
    expect(localStorage.getItem('avex_agent_token')).toBe('abc123')
    expect(getAuthToken()).toBe('abc123')
  })

  it('removes the token when cleared', () => {
    setAuthToken('abc123')
    setAuthToken(null)
    expect(localStorage.getItem('avex_agent_token')).toBeNull()
    expect(getAuthToken()).toBeNull()
  })
})

describe('apiFetch behaviour', () => {
  it('login uses the standard /auth/login endpoint and unwraps { data: ... }', async () => {
    fetchMock.mockResolvedValueOnce(
      mockResponse({ body: { data: { token: 't1', user: { id: 'a1' }, must_change_password: false } } })
    )

    const result = await agentAuthAPI.login({ phone: '0100', password: 'pw' })

    const [url, options] = fetchMock.mock.calls[0]
    expect(url).toBe('/api/v1/auth/login')
    expect(options.method).toBe('POST')
    expect(result.token).toBe('t1')
    expect(result.user.id).toBe('a1')
  })

  it('sends the Authorization header when a token is set', async () => {
    setAuthToken('tok-42')
    fetchMock.mockResolvedValueOnce(mockResponse({ body: { data: { success: true } } }))

    await agentAPI.assignTicket('t1', 'agent-1')

    const [, options] = fetchMock.mock.calls[0]
    expect(options.headers['Authorization']).toBe('Bearer tok-42')
  })

  it('on 401: clears the token (no redirect from apiFetch)', async () => {
    setAuthToken('expired')
    fetchMock.mockResolvedValueOnce(mockResponse({ status: 401 }))

    await expect(agentAPI.assignTicket('t1', 'agent-1')).rejects.toThrow()

    expect(getAuthToken()).toBeNull()
    // apiFetch no longer redirects — the auth store handles it via state.
  })

  it('extracts the error message from an { error } body', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ status: 500, body: { error: 'boom' } }))
    await expect(agentAPI.assignTicket('t1', 'agent-1')).rejects.toThrow('boom')
  })

  it('tickets list falls back gracefully on network failure (no crash)', async () => {
    fetchMock.mockRejectedValueOnce(new Error('network down'))
    const result = await agentAPI.getTickets()
    // FIXED: getTickets now returns { tickets, total } (was { tickets, agentId })
    expect(result).toEqual({ tickets: [], total: 0 })
  })
})
